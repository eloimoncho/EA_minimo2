...
if (process.env.NODE_ENV === 'production') {
  ...
  API_URL = "http://147.83.7.158:5432/rating";
} else {
  // Cargar variables de entorno desde .env.local
  API_URL = "http://localhost:5432/rating";
}


export class RatingsService {
  
  static async getRatingOf(idObject: string, type: string, ) {
    try {
      const response = await axios.get(API_URL + "/get/" + idObject + "/" + type, { headers: authHeader() });
